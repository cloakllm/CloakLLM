...
> **Audit status:** trusted timestamping is new in v0.11.0. It has had internal adversarial review (HIGH/MEDIUM findings fixed + regression-guarded), is validated cross-SDK against real RFC 3161 tokens, and as of **v0.11.1** its verifier is **differentially tested against `openssl ts -verify`** across a committed token corpus (both SDKs must reach OpenSSL's verdict) plus a DER-parser fuzz harness — v0.11.1 also added the RFC-mandated ESS SigningCertificateV2 check that the differential surfaced. It has **not yet had an independent third-party cryptographic audit**. Its tokens are standard RFC 3161 and verify with OpenSSL independently of CloakLLM, so you are never locked into our implementation. Before leaning a regulatory defence on this feature specifically, commission an independent crypto audit and use an eIDAS-qualified TSA — standard prudence for any compliance crypto, made cheap by CloakLLM being open and self-hosted.
